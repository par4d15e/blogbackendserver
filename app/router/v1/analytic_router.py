...
async def get_user_location_router(
    ...
    return SuccessResponse(message="用户地理位置数据获取成功", data=result)
...
async def get_blog_statistics_router(
    ...
    return SuccessResponse(message="博客统计数据获取成功", data=result)
...
async def get_tag_statistics_router(
    ...
    """获取标签统计（热门标签）"""
    result = await analytic_service.get_tag_statistics(
        role=current_user.role, limit=limit
    )
    return SuccessResponse(message="标签统计数据获取成功", data=result)
...
async def get_project_statistics_router(
    ...
    return SuccessResponse(message="项目统计数据获取成功", data=result)
...
async def get_payment_statistics_router(
    ...
    return SuccessResponse(message="支付统计数据获取成功", data=result)
...
async def get_top_ten_revenue_projects_router(
    ...
    return SuccessResponse(message="收入最高的前十项目数据获取成功", data=result)
...
async def get_user_statistics_router(
    ...
    return SuccessResponse(message="用户统计数据获取成功", data=result)
...
async def get_media_statistics_router(
    ...
    return SuccessResponse(message="媒体文件统计数据获取成功", data=result)
...
async def get_growth_trends_router(
    ...
    return SuccessResponse(message="增长趋势数据获取成功", data=result)
