...
@celery_app.task(
    name="large_content_translation_task", bind=True, max_retries=3, default_retry_delay=30
)
@with_db_init
def large_content_translation_task(self, content: dict, content_type: LargeContentTranslationType, content_id: int) -> bool:
    """
    ...
    try:
        ...
        # 验证内容类型
        if content_type not in [LargeContentTranslationType.BLOG, LargeContentTranslationType.PROJECT]:
        ...
        with mysql_manager.get_sync_db() as session:
            if content_type == LargeContentTranslationType.BLOG:
                ...
                blog_result = session.execute(
                    select(Blog).where(Blog.id == content_id)).first()
                if not blog_result:
                ...
                project_result = session.execute(
                    select(Project).where(Project.id == content_id)).first()
