...
class AnalyticService:
    ...
    async def get_tag_statistics(self, role: RoleType, limit: int = 20) -> List[Dict[str, Any]]:
        if role != RoleType.admin:
            raise HTTPException(
                status_code=403, detail="You are not authorized to access this resource")
    ...
    async def get_top_ten_revenue_projects(self, role: RoleType) -> List[Dict[str, Any]]:
        if role != RoleType.admin:
            raise HTTPException(
                status_code=403, detail="You are not authorized to access this resource")
...
def get_analytic_service(analytic_crud: AnalyticCrud = Depends(get_analytic_crud)) -> AnalyticService:
